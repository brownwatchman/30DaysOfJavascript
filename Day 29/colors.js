...
function generateColorCodes() {
  ...
  const filteredKeysOfColorCodes = Object.keys(colors)
    .filter((color) => color.includes(rangeOfShade))
...
const data = `const colorCodesRange900 = ${JSON.stringify(
  generateColorCodes()
)};
              const colorCodesRange300 = ${JSON.stringify(
                generateColorCodes()
              )};`;

// Write data to file in data folder
fileSystem.writeFile("colors.js", data, (err) => {
  if (err) throw err;
