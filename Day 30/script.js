...
function addCountries(countriesObject) {
  ...
  Object.keys(countriesObject).forEach((key) => {
    ...
    divElement.innerHTML = `
      ...
      <p class="country-languages">Languages: ${currentCountry.languages.join(
        ", "
      )}</p>
...
inputToSearchCountries.addEventListener("keyup", (e) => {
  ...
  const resultByName = Object.keys(countries_data)
    .map((key) => countries_data[key])
    .filter((country) => country.name.match(regExp));

  const resultByLanguage = Object.keys(countries_data)
    .map((key) => countries_data[key])
    .filter((country) => country.languages.join(" ").match(regExp));
...
function sortingMechanism(sortBy, isDescendingIsTrue) {
  ...
  const sortedCountries = Object.keys(currentResults)
    .map((key) => {
...
document.querySelectorAll("button").forEach((button) => {
  ...
  button.addEventListener("click", (e) => {
    ...
    if (e.target.classList.contains("sort-desc"))
      return sortingMechanism(e.target.textContent, 1);
...
let totalPopulation, totalLanguages;

(function findTotalPopulations() {
  const returnedObject = mostPopulatedCountries();

  const sumOfPopulation = Object.keys(returnedObject)
    .map((key) => {
...
(function findTotalLanguages() {
  ...
  const sumOfLanguages = Object.keys(returnedObject)
    .map((key) => {
...
function mostPopulatedCountries() {
  const mostPopulatedCountries = Object.keys(currentResults)
    .map((key) => {
...
function mostSpokenLanguages() {
  ...
  Object.keys(currentResults).forEach((key) => {
    currentResults[key].languages.forEach((lang) =>
      filteringLanguages.add(lang)
    );
  });

  const languagesWithCount = Array.from(filteringLanguages).reduce(
    (acc, cur) => {
      acc[cur] = {
        language: cur,
        count: 0,
  ...
  Object.keys(currentResults).forEach((key) => {
    currentResults[key].languages.forEach((lang) => {
      if (languagesWithCount[lang].language === lang)
        languagesWithCount[lang].count++;
    });
  });

  const tenMostSpokenLanguages = Object.keys(languagesWithCount)
    .map((key) => languagesWithCount[key])
    .sort((a, b) => {
